CREATE PROCEDURE "DBA"."Recall_Existencia_bago_ebc_rpt_materiales_Rpt2"( in mSID char(15),in mCod_Empresa char(2),in mFechaDesde date,in mFechaHasta date ) 
...
begin
  ...
  // DIAGNOSTICO RPT2:
  // Esta version se usa para reactivar la segunda pasada de manera
  // controlada, bloque por bloque, sin tocar la Rpt minima que ya pasa.
  ...
  // SEGUNDA PASADA - BLOQUE 1: VTA
  // APARTIR DE AQUI SE RECALCULA EL STOCK PARA LA COLUMNA RECUENTO PARA EL INFORME DE MATERIALES (EMPRESA INTERCOM)
  //--------------VENTAS------------------
  insert into dba.TmpRecallExistenciaEBC_Materiales
    select mSID,x.cod_empresa,m.cod_articulo,'VTA',sum(x.cantidad)
      from (
        select d.cod_empresa,d.cod_articulo,
          sum(if t.af_existencia = 'D' then
            d.cantidad*-1
          else if t.af_existencia = 'I' then
              d.cantidad
          ...
          from vtacab as c,vtadet as d,tpocbte as t
          where c.cod_empresa = d.cod_empresa
          and c.cod_tp_comp = d.cod_tp_comp
          and c.comp_numero = d.comp_numero
          and c.cod_empresa = t.cod_empresa
          ...
          and c.anulado = 'N'
          group by d.cod_empresa,d.cod_articulo
      ) as x,articulo as a,articulo as m
      ...
      and m.cod_familia = '99'
      and m.cod_grupo = '02'
      group by x.cod_empresa,m.cod_articulo
      order by x.cod_empresa asc,m.cod_articulo asc;
  // SEGUNDA PASADA - BLOQUE 2: REM
  //---------REMISIONES NO FACTURADAS-------------
  insert into dba.TmpRecallExistenciaEBC_Materiales
    select mSID,x.cod_empresa,m.cod_articulo,'REM',sum(x.cantidad)
      from (
        select d.cod_empresa,d.cod_articulo,sum(d.cantidad*-1) as cantidad
          from remiscab as c,remisdet as d,tpocbte as t
          where c.cod_empresa = d.cod_empresa
          and c.cod_tp_comp = d.cod_tp_comp
          and c.comp_numero = d.comp_numero
          and c.cod_empresa = t.cod_empresa
      ...
      group by x.cod_empresa,m.cod_articulo
      order by x.cod_empresa asc,m.cod_articulo asc;
  // SEGUNDA PASADA - BLOQUE 3: STK
  //---------------STOCK---------------
  insert into dba.TmpRecallExistenciaEBC_Materiales
    select mSID,x.cod_empresa,m.cod_articulo,'STK',sum(x.cantidad)
      from (
        select d.cod_empresa,d.cod_articulo,
          sum(if t.af_existencia = 'D' then
            d.cantidad*-1
          ...
          from stcab as c,stdet as d,tpocbte as t
          where c.cod_empresa = d.cod_empresa
          and c.cod_sucursal = d.cod_sucursal
          and c.cod_deposito = d.cod_deposito
          and c.cod_tp_comp = d.cod_tp_comp
      ...
      order by x.cod_empresa asc,m.cod_articulo asc;
  /*
  // SEGUNDA PASADA - BLOQUE 4: PRE/PRS/FCT/TRE/TRS
  //---------------PRODUCCION - ENTRADA------------
  insert into dba.TmpRecallExistenciaEBC_Materiales
    select mSID,c.cod_empresa,m.cod_articulo,'PRE',sum(c.cantidad)
      from ordprodcab as c,tpocbte as t,articulo as a,articulo as m
  ...
  insert into dba.TmpRecallExistenciaEBC_Materiales
    select mSID,d.cod_empresa,m.cod_articulo,'TRS',sum(d.cantidad*-1)
      ...
      group by d.cod_empresa,m.cod_articulo
      order by d.cod_empresa asc,m.cod_articulo asc;
  // SEGUNDA PASADA - BLOQUE 5: CONSOLIDACION RECUENTO
  //-----------INSERTO EN EL TEMPORAL--------------
  insert into tmp_ExistenciaEBC_Materiales( SessionID,cod_empresa,cod_articulo,Existencia_Calc ) 
    select mSID,d.cod_empresa,d.cod_articulo,sum(if d.cantidad is null then 0 else d.cantidad endif)
      from dba.TmpRecallExistenciaEBC_Materiales as d
