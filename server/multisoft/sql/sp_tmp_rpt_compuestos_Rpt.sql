CREATE PROCEDURE "DBA"."sp_tmp_rpt_compuestos_Rpt"( in msid char(15),in mcod_empresa char(2),in pd_fecha_inicio_desde date,
...
begin
  ...
  declare ln_cto_material_detalle numeric(16,4);
  ...
  declare lc_cursor_articulos_materiales_detalle dynamic scroll cursor for
    select articulopadredet.cod_articulo,
      sum(ordproddet.cantidad),
      articulopadredet.tipo_material_compuesto,
      coalesce(sum(coalesce(ordproddet.cantidad,0) * coalesce(ordproddet.cto_prom_gs,0)) / nullif(sum(coalesce(ordproddet.cantidad,0)),0),0)
  ...
  articulos_acondicionado_cabecera: loop fetch next lc_cursor_articulos_acondicionado_cabecera into ls_cod_tp_comp,ln_nroordprod,
    ...
    articulos_materiales_detalle: loop fetch next lc_cursor_articulos_materiales_detalle into ls_cod_art_material,ln_cant_art_material,ls_tipo_material,ln_cto_material_detalle;
      if(sqlstate = no_data_found) then
        leave articulos_materiales_detalle
      end if;
      set ln_cto_material = coalesce(ln_cto_material_detalle,0);
      if coalesce(ln_cto_material,0) = 0 then
      ...
      if coalesce(ln_cto_material,0) = 0 then
        select coalesce(avg(aux.costo_ut_rep),0)
          into ln_cto_material
          from tmp_ebc_materiales_costo_aux as aux,articulo as art_hijo
          where aux.cod_articulo = art_hijo.cod_articulo
          and art_hijo.cod_empresa = mcod_empresa
          and art_hijo.codartpad = ls_cod_art_material
          and aux.fecha_ini = pd_fecha_fin_desde
          and aux.fecha_fin = pd_fecha_fin_hasta;
      end if;
      if coalesce(ln_cto_material,0) = 0 then
        select top 1 coalesce(costo_ut_rep,0)
          into ln_cto_material
          from tmp_ebc_materiales_costo_aux
          where cod_articulo = ls_cod_art_material
          and fecha_ini > pd_fecha_fin_hasta
          order by fecha_ini asc,fecha_fin asc;
      end if;
      if coalesce(ln_cto_material,0) = 0 then
        select top 1 coalesce(avg(aux.costo_ut_rep),0)
          into ln_cto_material
          from tmp_ebc_materiales_costo_aux as aux,articulo as art_hijo
          where aux.cod_articulo = art_hijo.cod_articulo
          and art_hijo.cod_empresa = mcod_empresa
          and art_hijo.codartpad = ls_cod_art_material
          and aux.fecha_ini > pd_fecha_fin_hasta
          group by aux.fecha_ini,aux.fecha_fin
          order by aux.fecha_ini asc,aux.fecha_fin asc;
      end if;
      if coalesce(ln_cto_material,0) = 0 then
        select coalesce(cto_prom_gs,0)
          into ln_cto_material
          from tmp_cto_ult_dia_mes
          where cod_empresa = mcod_empresa
          and fecha = pd_fecha_fin_hasta
          and cod_articulo = ls_cod_art_material;
      end if;
      if coalesce(ln_cto_material,0) = 0 then
        select coalesce(avg(aux.cto_prom_gs),0)
          into ln_cto_material
          from tmp_cto_ult_dia_mes as aux,articulo as art_hijo
          where aux.cod_articulo = art_hijo.cod_articulo
          and aux.cod_empresa = mcod_empresa
          and aux.fecha = pd_fecha_fin_hasta
          and art_hijo.cod_empresa = mcod_empresa
          and art_hijo.codartpad = ls_cod_art_material;
      end if;
      if coalesce(ln_cto_material,0) = 0 and mcod_empresa = 'IN' then
        select coalesce(cto_prom_gs,0)
          into ln_cto_material
          from tmp_cto_ult_dia_mes
          where cod_empresa = 'BG'
          and fecha = pd_fecha_fin_hasta
          and cod_articulo = ls_cod_art_material;
      end if;
      if coalesce(ln_cto_material,0) = 0 and mcod_empresa = 'IN' then
