...
CentroCostos.all = function (params, cb) {
    var string = "SELECT DBA.ACUMAUXI.CodPlanAux,UPPER (DBA.PLANCTA.CodPlanCta) AS CodPlanCta,DBA.PLANCTA.Nombre,"+
                 "DBA.PLANCTA.TipoSaldo,CAST(sum(DBA.ACUMAUXI.TotalDbME) as decimal(20,0)) as sum_TotalDbME,"+
                 "CAST(sum(DBA.ACUMAUXI.TotalCrME) as decimal(20,0)) as sum_TotalCrME,"+
                 ...
                 "DBA.ACUMAUXI.Cod_Empresa = DBA.PLANCTA.Cod_Empresa AND DBA.ACUMAUXI.Periodo = DBA.PLANCTA.Periodo "+ 
                 "AND DBA.ACUMAUXI.CodPlanCta = DBA.PLANCTA.CodPlanCta AND DBA.ACUMAUXI.Cod_Empresa = DBA.PLANAUXI.Cod_Empresa "+    
                 "AND DBA.ACUMAUXI.Periodo = DBA.PLANAUXI.Periodo AND DBA.ACUMAUXI.CodPlanCta = DBA.PLANAUXI.CodPlanCta "+
                 "AND DBA.ACUMAUXI.CodPlanAux = DBA.PLANAUXI.CodPlanAux AND DBA.ACUMAUXI.Cod_Empresa = '"+params.empresa+"' "+
                 "AND DBA.ACUMAUXI.Periodo = '"+params.periodo+"' AND DBA.ACUMAUXI.AnhoMes >= '"+params.periodo+params.mesd+"' "+
                 "AND DBA.ACUMAUXI.AnhoMes <= '"+params.periodo+params.mesh+"' AND DBA.PLANAUXI.Imputable = 'S' " 
                 if (params.nivel > 0)
