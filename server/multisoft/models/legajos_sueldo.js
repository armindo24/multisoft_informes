...
Legajos.all = function (filters, cb) {
    ...
    var select = "dba.legajo.cod_empresa, dba.legajo.codempleado, dba.legajo.fecha, dba.legajo.codmvto, dba.legajo.aprobado, dba.legajo.sucanterior, "+           
                 "dba.legajo.dptoanterior, dba.legajo.cargoanterior, dba.legajo.sueldoanterior, dba.legajo.sucactual, dba.legajo.dptoactual, "+           
                 "dba.legajo.cargoactual, dba.legajo.sueldoactual, dba.legajo.porcvariacion, dba.legajo.observacion, dba.empleados.apellidos, "+           
                 "dba.empleados.nombres, dba.empleados.cedula, date(dba.empleados.fechanac) as fechanac, dba.empleados.sexo, case when dba.empleados.estcivil = 'S' then 'Soltero' else (case when dba.empleados.estcivil = 'C' then 'Casado' else (case when dba.empleados.estcivil = 'D' then 'Divorciado' else (case when dba.empleados.estcivil = 'O' then 'Otros' end) end) end) end as estcivil, "+ 
                 "case when dba.empleados.aportaips = 'S' then 'Si' else 'No' end as aportaips,"+           
                 "dba.empleados.nroips, dba.empleados.direccion, isnull(dba.empleados.telefono,'',dba.empleados.telefono) as telefono, date(dba.empleados.fechaentrada) as fechaentrada, dba.empresa.des_empresa, "+
                 "dba.mvtolegajo.descrip, sucursal_a.des_sucursal, dpto_a.descrip, cargos_a.descrip, sucursal_b.des_sucursal, dpto_b.descrip, cargos_b.descrip ";
    ...
    sql += " ORDER BY dba.legajo.codempleado";
