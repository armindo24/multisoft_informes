...
type DetailState = {
  ...
  loaded: boolean;
  rows: AuxDetailRow[];
  saldoAnterior: AuxSaldoRow | null;
  error: string | null;
};
...
function buildExportData(params: {
  ...
  const headers =
    params.moneda === 'ambas'
      ? ['Cuenta', 'Nombre cuenta', 'Auxiliar', 'Nombre auxiliar', 'Fecha', 'Tipo', 'Nro. Comp.', 'Nro.', 'Linea', 'Cod. Auxiliar', 'Concepto', 'Debe M.L.', 'Haber M.L.', 'Saldo M.L.', 'Debe M.E.', 'Haber M.E.', 'Saldo M.E.']
      : ['Cuenta', 'Nombre cuenta', 'Auxiliar', 'Nombre auxiliar', 'Fecha', 'Tipo', 'Nro. Comp.', 'Nro.', 'Linea', 'Cod. Auxiliar', 'Concepto', 'Debe', 'Haber', 'Saldo'];

  const rows: Array<Array<unknown>> = [];

  for (const group of params.groups) {
    for (const aux of group.auxiliares) {
      const key = `${aux.accountCode}:${aux.auxCode}`;
      const state = params.states[key];
...
export function MayorCuentaAuxPanel({
  groups,
  empresa,
  periodo,
  fechad,
  ...
  async function loadAuxDetail(aux: AuxAccount) {
    const key = `${aux.accountCode}:${aux.auxCode}`;
    ...
    try {
      const url = new URL('/api/finanzas/mayor-cuenta-aux-det', window.location.origin);
      ...
      url.searchParams.set('cuenta', aux.auxCode);
      url.searchParams.set('path', aux.accountCode);
  ...
  async function toggleAux(aux: AuxAccount) {
    const key = `${aux.accountCode}:${aux.auxCode}`;
    const isOpen = openAux === key;
    setOpenAux(isOpen ? null : key);
    if (isOpen) return;
    await loadAuxDetail(aux);
  }

  function prefetchAux(aux: AuxAccount) {
    void loadAuxDetail(aux);
  }

  function exportExcel() {
    exportRowsToExcel({
      title: 'Libro mayor de auxiliares',
  ...
  return (
    <section id="mayor-cuenta-aux" className="scroll-mt-28 space-y-4">
      <div className="card flex flex-col gap-3 px-5 py-4 md:flex-row md:items-start md:justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-900">Libro mayor de auxiliares</h2>
          <p className="mt-1 text-sm text-slate-500">Agrupa auxiliares por cuenta y carga el detalle solo al abrir cada auxiliar para mantener el informe mas liviano.</p>
        </div>
        {hasExportRows ? (
        ...
        groups.map((group) => (
          <div key={group.accountCode} className="card overflow-hidden">
            <div className="border-b border-slate-200 bg-slate-50 px-5 py-4">
              <h3 className="text-base font-semibold text-slate-900">{group.accountCode} {group.accountName}</h3>
            </div>
            <div className="space-y-3 px-4 py-4">
              {group.auxiliares.map((aux) => {
                const key = `${aux.accountCode}:${aux.auxCode}`;
                ...
                return (
                  <div key={key} className="rounded-2xl border border-slate-200 bg-white">
                    <button
                      type="button"
                      onClick={() => void toggleAux(aux)}
                      onMouseEnter={() => prefetchAux(aux)}
                      onFocus={() => prefetchAux(aux)}
                      className="flex w-full items-center justify-between px-4 py-3 text-left"
                    >
                      <span className="font-medium text-slate-900">{aux.auxCode} {aux.auxName}</span>
                            ...
                            <table className="min-w-full text-sm">
                              ...
                              <tbody>
                                <tr className="border-b border-slate-100 bg-slate-50">
                                  <td colSpan={7} className="px-3 py-2 text-right font-semibold">Saldo anterior</td>
                              ...
                              <tfoot>
                                <tr className="bg-slate-50 font-bold text-slate-900">
                                  <td colSpan={7} className="px-3 py-3 text-right">Totales del periodo</td>
